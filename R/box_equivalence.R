#' Mapping Between Regular SSM Grid Cells and Atlantis Polygons
#'
...
#'
#' @param SSM_file_path Path to a SSM file. Used to load `siglev` and `siglay` information.
#' @param SSM_file_name Name of the SSM file (Hydrologic file).
#' @param RegularSSM_file_path Path to the SSM file with a regular grid (after interpolation).
#' @param RegularSSM_file_name Regular grid SSM file containing TS (temperature/salinity) variables.
#'
#' @return No value is returned. One CSV file is written in `R/code`:
#' `box_composition.csv`.
#'
#' @examples
...
#' RegularSSM_file_name <- "regular_grid_TS_novelma_2011.nc"
#' box_equivalence(SSM_file_path, SSM_file_name,
#'                             RegularSSM_file_path, RegularSSM_file_name)
#'
#' @export

box_equivalence <- function(SSM_file_path,SSM_file_name,
                            RegularSSM_file_path,RegularSSM_file_name) {
  select <- dplyr::select
  map <- purrr::map
...
# join ROMS grids with Atlantis boxes (rho grid)
# Boxes
