...
RDON_dim <- roms_vars %>% dplyr::filter(name==c("RDON")) %>% purrr::pluck('grd')
...
foreach::foreach(days = step_file,
  ...
  RDON <- ncdf4::ncvar_def("RDON", "double", dim = list( z_dim,b_dim, t_dim),
  ...
  nc <- ncdf4::nc_create(nc_filename, vars = list(RDON = RDON, LDON = LDON))
  ...
  ncdf4::ncvar_put(nc, RDON, atlantis_input_RDON, start = c(1,1,1),count = c( layer+1,box, length(time)))
  ncdf4::ncvar_put(nc, LDON, atlantis_input_LDON, start = c(1,1,1),count = c( layer+1,box, length(time)))

  # Add minimum and maximum values to RDON variable attributes
  ncdf4::ncatt_put(nc, "RDON", "valid_min", -50)
  ncdf4::ncatt_put(nc, "RDON", "valid_max", 200)

  # Add minimum and maximum values to LDON variable attributes
  ncdf4::ncatt_put(nc, "LDON", "valid_min", 0)
  ncdf4::ncatt_put(nc, "LDON", "valid_max", 2000)
