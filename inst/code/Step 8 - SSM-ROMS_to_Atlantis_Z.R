...
SZ_dim <- roms_vars %>% dplyr::filter(name==c("SZ")) %>% purrr::pluck('grd')
...
foreach::foreach(days = step_file,
  ...
  SZ <- ncdf4::ncvar_def("SZ", "double", dim = list( z_dim,b_dim, t_dim),
                  units = "mgN.m-3", missval = NA, longname = "SZ")
  LZ <- ncdf4::ncvar_def("LZ", "double", dim = list( z_dim,b_dim, t_dim),
  ...
  nc <- ncdf4::nc_create(nc_filename, vars = list(SZ = SZ, LZ = LZ))
  ...
  ncdf4::ncatt_put(nc, "SZ", "valid_min", -50)
  ncdf4::ncatt_put(nc, "SZ", "valid_max", 200)

  # Add minimum and maximum values to LZ variable attributes
  ncdf4::ncatt_put(nc, "LZ", "valid_min", 0)
  ncdf4::ncatt_put(nc, "LZ", "valid_max", 2000)
