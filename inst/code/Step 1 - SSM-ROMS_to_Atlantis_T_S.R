...
temperature_dim <- roms_vars %>% dplyr::filter(name==c("temp")) %>% purrr::pluck('grd')
...
foreach::foreach(days = step_file,
  .packages = c("dplyr", "tidync", "ncdf4", "purrr", "magrittr"),
  ...
  t_dim <- ncdf4::ncdim_def("t",paste0("seconds since ",year,"-01-01"), (time-1)*60*60)
  # Define variables
  ...
  nc <- ncdf4::nc_create(nc_filename, vars = list(temperature = temperature, salinity = salinity))
