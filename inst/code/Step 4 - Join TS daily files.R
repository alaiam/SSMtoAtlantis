...
for (i in 1:length(list.file)){
  nc <- nc_open(paste0(input_path, "/Physical_var_AtlantisTS_",i,".nc"))
...
t_dim <- ncdim_def("t",units = paste0("seconds since ",year,"-01-01"), time, unlim = T)
...
###################################################################################
# Define dimensions
t_dim <- ncdim_def("t", paste0("seconds since ",year,"-01-01"), time, unlim = T)
z_dim <- ncdim_def("z","layerNum", 1:(layer+1))
b_dim <- ncdim_def("b","boxNum", 0:(box-1))
